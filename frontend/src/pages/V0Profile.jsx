...
const defaultUserData = {
  name: "未登录", level: "普通会员", levelProgress: 0,
  points: 0, balance: 0, coupons: 0, favorites: 0,
  phone: "",
}
...
function getAssets(ud) {
  return [
    { name: "账户余额", value: `¥${ud.balance}`, icon: Wallet, color: "from-orange-400 to-orange-500" },
    { name: "充值", value: "充值", icon: CreditCard, color: "from-green-400 to-emerald-500" },
    { name: "优惠券", value: `${ud.coupons}张`, icon: Ticket, color: "from-pink-400 to-pink-500" },
    { name: "积分", value: ud.points, icon: Zap, color: "from-purple-400 to-purple-500" },
    { name: "收藏", value: ud.favorites, icon: Heart, color: "from-red-400 to-red-500" },
  ]
}
...
export default function V0Profile() {
  ...
  const userData = user ? { ...defaultUserData, name: user.name, phone: user.email } : defaultUserData
      ...
      <div className="container mx-auto px-4 -mt-16 lg:-mt-20 relative z-10 pb-24 lg:pb-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 lg:p-6 mb-4">
          <div className="grid grid-cols-5 gap-1 lg:gap-4">
            {getAssets(userData).map((item, index) => (
        ...
        <button onClick={handleLogout} className="w-full bg-white rounded-2xl shadow-sm p-4 flex items-center justify-center gap-2 text-gray-500 hover:text-red-500 hover:bg-red-50 transition-colors">
